...
var onLoad = function() {
  if (chatroom.messages.length > 0) {
    chatroom.messages.forEach(function(message, index) {
      if (index % 2 === 0) {
        debugger
        $('.chat').prepend(`<p class="user-message" data-id=${index}>${message.text}<p><button>Delete</button>`);
      } else {
...
function addUserMessage(){
  ...
  $('.chat').prepend(
    `
      <p class="user-message" data-id=${id}>${$userInput.val()}<p><button>Delete</button>
    `
  );
};
