...
var countMessagesOnDOM = function() {
  var counter = 0
  $('.chat').children().each(function(message) {
...
var showAllMessageButton = function() {
  if ($('.chat').children().length > 9) {
    $('.show-messages').attr('hidden', false)
    $('.show-messages').on('click', function() {
      showAllMessages()
    })
  } else if (countMessagesOnDOM() < 10) {
    $('.show-messages').attr('hidden', true)
  }
}

var showTenMessages = function() {
  $($('.chat').children().toArray().reverse()).each(function(index, message) {
    if (index > 9) {
...
var showAllMessages = function() {
  $('.chat').children().each(function(index, message) {
  ...
  if (chatroom.messages.length > 0) {
    chatroom.messages.forEach(function(message, index) {
      if (typeof message === 'object') {
        ...
        delete-button" data-id=${index}><strong>X</strong></button></section>`)
...
$('.send-button').on('click', function(){
  ...
  addUserMessage()
  $userInput.val('')
  $('.character-counter').text(`Counter: 0`)
  disableSend()
  setTimeout(function(){
    addBotMessage()
